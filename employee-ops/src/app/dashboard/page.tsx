...
export default function EmployeeOps() {
  ...
  const getStatus = (daysLeft: number) => {
    if (daysLeft <= 7)
      return {
        label: "Critical",
        style: "bg-[#2d2d4f] text-[#3ecf73]",
      };

    if (daysLeft <= 30)
      return {
        label: "Expiring Soon",
        style: "bg-[#e2ded6] text-[#1f1f38]",
      };

    return {
      label: "Compliant",
      style: "bg-[#3ecf73]/15 text-[#2fb866]",
    };
  };
    ...
    <div className="px-4 sm:px-8 lg:px-12 py-8 max-w-7xl mx-auto bg-[#f6f4ef] min-h-screen space-y-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
        ...
        <div className="bg-white p-6 sm:p-8 rounded-3xl border border-[#e2ded6] hover:border-[#2d2d4f] transition-all duration-300 shadow-sm hover:shadow-md">
          <h3 className="font-bold text-[#1f1f38] mb-6">
      ...
      <div className="bg-white rounded-[40px] border border-[#e2ded6] overflow-hidden shadow-sm">
        ...
        <div className="px-6 sm:px-10 py-6 border-b border-[#e2ded6] bg-[#f6f4ef]">
          <h3 className="text-lg font-bold text-[#1f1f38]">
            Compliance Registry and Overview
          </h3>
        </div>
            ...
            <tbody className="divide-y divide-[#e2ded6]">
              {staffData.map((staff) => {
                ...
                return (
                  <tr
                    key={staff.name}
                    className="hover:bg-[#f6f4ef] transition-all duration-200"
                  >
                    <td className="px-6 sm:px-10 py-6 font-bold text-[#1f1f38]">
                      {staff.name}
                    </td>
                    <td className="px-6 sm:px-10 py-6 text-[#2d2d4f] font-medium">
                      {staff.role}
                    </td>
                    <td className="px-6 sm:px-10 py-6 text-center font-semibold text-[#1f1f38]">
                      {staff.daysLeft} days
                    </td>
                    <td className="px-6 sm:px-10 py-6 text-center">
                      <span
                        className={`px-5 py-2 rounded-full text-[10px] font-black tracking-widest ${status.style}`}
                      >
                        {status.label}
                      </span>
                    </td>
...
function KPI({ label, value }: { label: string; value: number }) {
  return (
    <div className="bg-white p-5 sm:p-6 rounded-2xl border border-[#e2ded6] hover:border-[#2d2d4f] transition-all duration-300 shadow-sm hover:shadow-md">
      ...
      <h2 className="text-2xl sm:text-3xl font-black mt-2 text-[#1f1f38]">
        {value}
      </h2>
    </div>
  );
