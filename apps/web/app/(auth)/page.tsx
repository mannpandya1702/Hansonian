...
// Fallback: demo mode when Firebase is not yet configured
// ============================================================
...
// Role is stored as a custom claim on the Firebase user token.
// Until the backend sets custom claims, we derive role from email domain/prefix.
function deriveRole(email: string): "admin" | "employee" | "family" | null {
  const e = email.toLowerCase().trim()
  if (e.startsWith("admin@") || e.startsWith("ceo@"))      return "admin"
  if (e.startsWith("employee@") || e.startsWith("staff@")) return "employee"
  if (e.startsWith("family@") || e.startsWith("participant@")) return "family"
  return null
}

// Demo fallback — only active when NEXT_PUBLIC_FIREBASE_API_KEY is not set
...
const ROLE_META = {
  admin:    { label: "CEO / Admin",        description: "Financial pulse, DEX compliance & strategic alerts",   badgeCls: "bg-[#4ade80]/15 text-[#16a34a] border border-[#4ade80]/40" },
  employee: { label: "Employee",           description: "Rostering, compliance vault & session review",         badgeCls: "bg-[#1a1a2e]/10 text-[#1a1a2e] border border-[#1a1a2e]/20" },
  family:   { label: "Family / Participant", description: "Budget monitor, visit feed & caregiver ratings",     badgeCls: "bg-[#e8e4dd] text-[#4a4a6a] border border-[#d0ccc4]" },
} as const
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      if (isFirebaseConfigured) {
        ...
        const role = (tokenResult.claims.role as string) ?? deriveRole(email)

        if (!role || !(role in PORTAL_PATHS)) {
          setError("Your account does not have an assigned portal role. Contact your administrator.")
          setLoading(false)
          return
        }
        ...
        // ── Demo fallback (no Firebase credentials yet) ─────
        ...
        <div className="relative z-10 space-y-8">
          <div>
            <h1 className="text-4xl xl:text-[2.6rem] font-semibold text-white leading-snug" style={{ fontFamily: "var(--font-playfair, Georgia, serif)" }}>
              Intelligent NDIS<br />Care Management
            </h1>
          ...
          <div className="space-y-2.5">
            {(["admin", "employee", "family"] as const).map((role) => (
              <button
                ...
                className="w-full flex items-center gap-3 bg-white/5 border border-white/10 rounded-xl px-4 py-3 hover:bg-white/9 hover:border-white/20 transition-all text-left"
              >
      ...
      <div className="flex-1 flex flex-col items-center justify-center bg-[#faf9f7] min-h-screen lg:min-h-0 px-5 py-10 sm:px-10">
        ...
        <div className="w-full max-w-[420px]">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            {/* Error */}
            {error && (
              <div role="alert" className="flex items-center gap-2 text-sm text-[#c0392b] bg-red-50 border border-red-100 px-4 py-3 rounded-xl">
                ...
                {(["admin", "employee", "family"] as const).map((role) => {
                  ...
                  return (
                    <button
                      key={role}
                      onClick={() => fillDemo(role)}
                      className="w-full flex items-center justify-between px-3 py-2.5 bg-white border border-[#e8e4dd] rounded-lg hover:border-[#1a1a2e]/30 hover:shadow-sm transition-all text-left"
                    >
                      <div>
