...
export default function ComplianceVault() {
  ...
  const calculateStatus = (credentials: { type: string; expiry: string | null }[]) => {
    ...
    credentials.forEach((cred) => {
      if (!cred.expiry) { hasMissing = true; return; }
                  ...
                  <tr key={staff.name} className="hover:bg-[#faf9f7] transition-colors">
                    ...
                    <td className="px-6 sm:px-10 py-5 space-y-2">
                      {staff.credentials.map((cred, index) => {
                        ...
                        return (
                          <div key={index} className={`text-xs font-semibold ${isExpiring ? "text-[#b45309]" : "text-[#4a4a6a]"}`}>
                            {cred.type} — Expires {expiryDate.toLocaleDateString()}
                          </div>
                        );
                      })}
                    </td>
                    <td className="px-6 sm:px-10 py-5 text-center">
                      <span className={`px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest ${status.style}`}>
                        {status.label}
                      </span>
                    </td>
                  </tr>
